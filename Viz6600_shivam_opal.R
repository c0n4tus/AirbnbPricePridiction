...
library(tidyverse)
df_new10 <- read_csv(file="appData.csv")

#Partitioning the dataset into training and test data
set.seed(111)
#Generate a random number that is 80% of the total number of rows in dataset.
train.index4 <- sample(row.names(df_new10), 0.8*dim(df_new10)[1])  
valid.index4 <- setdiff(row.names(df_new10), train.index4)  
train.df4 <- df_new10[train.index4,]
valid.df4 <- df_new10[valid.index4,]


validate <- data.frame('property_type.Boat' = 1, 'property_type.Boutique.hotel' = 0, 'property_type.Condominium' = 0,
                      'property_type.Dorm' = 0, 'property_type.Guest.suite' = 0, 'property_type.House'=0,
                      'property_type.In.law'= 0, 'property_type.Loft'= 0, 'property_type.Other'= 0,
                      'property_type.Timeshare' = 0,
                      'room_type.Private.room' = 1, 'room_type.Shared.room'= 0,
                      "number_of_reviews" = 50, "bathrooms" = 1, "review_scores_rating" = 80,
                      "beds" = 1, "city" = "NYC",
                      "Essentials" = 0, "Family_kid_friendly" = 0, "Hair_dryer" = 0, "Hangers" = 0, "Heating" = 1,
                      "Laptop_friendly_workspace" = 0, "Shampoo" = 0, "Smoke_detector" = 0, "Washer" = 1, "Air_conditioning" = 1)


# Fit the linear model
fit_lm4 <-lm(price~., data = train.df4)
# Predict testing data on our linear model
#summary(fit_lm4)
#pred<- predict(fit_lm, valid.df)
#make the prediction
#pred_lm4 <- predict(fit_lm4, newdata = validate, interval = "confidence")
#residuls4 <- valid.df4$price[1:15]-pred_lm4[1:15]
# d4 <- data.frame("Predicted"= pred_lm4[1:15], "Actual"=valid.df4$price[1:15], "Residul"= residuls4)
# #randomly samples 10 rows from the dataframe to display
# library(knitr)
# kable(sample_n(d4, 15))
# 
# 
# 
# RMSE(pred_lm4,valid.df4$price) 
#pred_lm4
